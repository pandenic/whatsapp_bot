...
async def validate_create_repeatable_reminder_command(
    ...
    try:
        ...
        if (not reminder_text_list[0].isdigit() and
                0 > int(reminder_text_list[0]) > repeat_scenarios_quantity):
...
async def validate_selector_choice(
    ...
    try:
        if not choice.isdigit() or int(choice) >= len(list(Selector)):
            raise e.WrongSelectorValue(f"Wrong selector value.\nChoose a number between 1 and {len(Selector.get_list())-1}")
...
async def validate_reminder_delete_number(
    ...
    try:
        if not value.isdigit() or int(value) < 0:
            raise e.WrongReminderNumber("Write a number. It should be greater than or equal to 0.")
    except e.WrongReminderNumber as error:
        return error


async def validate_reminder_number_is_exist(
        number: int,
        quantity: int
):
    try:
        if quantity <= number:
            raise e.WrongReminderNumber(f"Reminder with this number does not exist.\n"
